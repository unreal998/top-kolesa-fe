...
export function ImgCarousel ({sliderData, ItemElement, gap, innerWidth, outerWidth}: IImgCarousel) {
    ...
    const onTouchMove = useCallback((touchStartEvent: any) => {
        if (isMouseDown) {
           if (initDragablePosition - touchStartEvent.touches[0].clientX > 100) {
                onSwipeLeft()
                setMouseDown(false);
           } else if (touchStartEvent.touches[0].clientX > initDragablePosition && initDragablePosition - touchStartEvent.touches[0].clientX > -100) {
                onSwipeRight()
                setMouseDown(false);
           }
        }
    ...
    return (
        ...
        >
                <Stack 
                    ...
                    >
                        {
                            sliderData.map( (item, i) => <ItemElement key={i} imgSource={item.imgSource} description={item.description} hoverDescription={item.hoverDescription} /> )
                        }
                </Stack>
        </Box>
